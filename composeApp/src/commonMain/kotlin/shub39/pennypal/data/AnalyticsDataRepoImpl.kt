...
class AnalyticsDataRepoImpl(
    ...
    override fun getAnalyticsData(): Flow<AnalyticsData> {
        return combine(
            ...
            val transactionsWithMonth = transactions.map {
            ...
            // Avg Monthly Income
            val monthlyIncome = transactionsWithMonth
                .filter { it.first.transactionType == TransactionType.INCOME }
                .groupBy { it.second }
                .mapValues { it.value.sumOf { t -> t.first.amount } }
            val avgMonthlyIncome = if (monthlyIncome.isNotEmpty()) monthlyIncome.values.average() else 0.0

            // Avg Monthly Expense
            val monthlyExpense = transactionsWithMonth
                .filter { it.first.transactionType == TransactionType.EXPENSE }
                .groupBy { it.second }
                .mapValues { it.value.sumOf { t -> t.first.amount } }
            val avgMonthlyExpense = if (monthlyExpense.isNotEmpty()) monthlyExpense.values.average() else 0.0

            // Pie Chart Data (All time)
            val incomePieChartData = incomeTransactions
            ...
            val expensePieChartData = expenseTransactions
                .groupBy { it.categoryId }
                .map { (catId, trans) ->
                    val category = categoryMap[catId]
                    CategoryAmount(
            ...
            val sortedMonths = transactionsWithMonth
                .map { it.first.date to it.second }
                .sortedBy { it.first }
                .map { it.second }
                .distinct()

            val incomeVsExpenseData = sortedMonths.map { month ->
            ...
            val expenseCategories = expenseTransactions.mapNotNull { categoryMap[it.categoryId] }.distinctBy { it.id }
            val expenseCategoriesComparison = expenseCategories.map { category ->
                val values = sortedMonths.map { month ->
                    transactionsWithMonth
                        .filter { it.second == month && it.first.categoryId == category.id && it.first.transactionType == TransactionType.EXPENSE }
